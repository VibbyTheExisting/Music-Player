...
class MusicPlayer(App):
    def build(self):
        ...
        d = {"Add to Queue": lambda x: self.playSong(
            x, "Q"), "Playlists": lambda x: self.playlistsGUI(), "loop (off)": self.toggleLoop, "Queue":lambda x: self.viewQueue()}
        for key, val in d.items():
        ...
        self.timeSlider = Slider(min=0, max=0, step=1,
        ...
        speedSlider = Slider(min=0.5, max=2, step=0.05,
        ...
        volSlider = Slider(min=0, max=100, step=1,
    ...
    def viewQueue(self):
        self.man.current = "queue"
        self.man.current_screen.clear_widgets()
        d = {}
        layout = BoxLayout(orientation='vertical')
        if self.queue:
            images = len(self.queue) <= 8
            l = self.queue.copy()
            ...
            for songList in l:
                ...
                remove = Button(text="Remove", size_hint_x=None, size_hint_y=None, width=70, height=20, size=(70,20))
                d[str(remove)] = songList
                remove.bind(on_release=lambda button1: self.queue.remove(d[str(button1)]) or self.backToMain(None, ""))
                subLayout.add_widget(remove)
                tot.add_widget(subLayout)
                layout.add_widget(tot)
        inner = BoxLayout(orientation='horizontal')
        b = Button(text="Back", size_hint_x=None, size_hint_y=None,
                width=50, height=20, size=(50, 20))
        b.bind(on_release=lambda x: self.backToMain(None, ""))
        inner.add_widget(b)
    ...
    def showResults(self, text, _type):
        ...
        for result in res:
            ...
            with yt_dlp.YoutubeDL({'extract_audio': True, 'format': 'bestaudio', 'outtmpl': '%(title)s.mp3'}) as video:
                while True:
                    try:
                        info = video.extract_info(
                            f"https://www.youtube.com/watch?v={result}", download=False)
                        break
            ...
            self.playDict[b] = [
                f"https://www.youtube.com/watch?v={result}", info['title'], info['uploader'], info['thumbnail']]
            a.add_widget(b)
            subLayout.add_widget(a)
            tot.add_widget(subLayout)
    ...
    def playlistsGUI(self):
        ...
        but = Button(text="Back", size_hint_x=None,
                     size_hint_y=None, width=40, height=10, size=(40, 10))
        but.bind(on_release=lambda x: self.backToMain(None, ""))
        horz.add_widget(but)
        add = Button(text="Add", size_hint_x=None, size_hint_y=None,
                     width=40, height=10, size=(40, 10))
        ...
        for name, songs in self.playlists.items():
            ...
            b = Button(text="View", size_hint_x=None,
                       size_hint_y=None, width=40, height=15, size=(40, 15))
            b.bind(on_release=self.playlistSingularGUI)
            self.playlistButtonDict[str(b)] = store
            inner.add_widget(b)
            b = Button(text="Play", size_hint_x=None,
                       size_hint_y=None, width=40, height=15, size=(40, 15))
            b.bind(on_release=self.playlistPlay)
            self.playlistButtonDict[str(b)] = store
            inner.add_widget(b)
            ba = Button(text="Shuffle", size_hint_x=None,
                        size_hint_y=None, width=60, height=15, size=(60, 15))
            ba.bind(on_release=self.playlistPlay)
            self.playlistButtonDict[str(ba)] = store
            inner.add_widget(ba)
            b = Button(text="Remove", size_hint_x=None,
                       size_hint_y=None, width=60, height=15, size=(60, 15))
    ...
    def playlistSingularGUI(self, button):
        ...
        for song in self.playlists[self.playlistButtonDict[str(button)][0]]:
            ...
            subLayout.add_widget(innerLayout)
            subLayout.add_widget(remove)
            self.playlistButtonDict[str(remove)] = [
                self.playlistButtonDict[str(button)][0], str(song)]
            tot.add_widget(subLayout)
            layout.add_widget(tot)
        inner = BoxLayout(orientation='horizontal')
        b = Button(text="Back", size_hint_x=None, size_hint_y=None,
        ...
        inner.add_widget(b)
        b = Button(text="Play", size_hint_x=None, size_hint_y=None,
                   width=50, height=20, size=(50, 20))
        b.bind(on_release=lambda x: self.playlistPlay(button))
        inner.add_widget(b)
        b = Button(text="Shuffle", size_hint_x=None,
                   size_hint_y=None, width=80, height=20, size=(80, 20))
        b.bind(on_release=lambda x: self.playlistPlay(button, shuffle=True))
        inner.add_widget(b)
        b = Button(text="Add", size_hint_x=None, size_hint_y=None,
                   width=40, height=20, size=(40, 20))
        b.bind(on_release=lambda x: self.addToPlaylist(
            self.playlistButtonDict[str(button)][0]))
        inner.add_widget(b)
    ...
    def updateLabel(self):
        try:
            self.currentLabel.text = self.info.title if len(
                self.info.title) < 40 else self.info.title[:36] + "..."
    ...
    def checking(self):
        if not self.med.is_playing():
            if self.paused or self.doingSomething:
                pass
            elif self.loop and self.info and self.info.url:
                self.repeat()
    ...
    def backToMain(self, button, _type):
        if _type == "Play":
            ...
            Clock.schedule_once(lambda x: self.resetLoopAndPause(
            ) or self.play(self.playDict[button]), 1)
        elif _type == "Q":
            self.playingPlaylist = False
            self.man.current = 'main'
            self.queue.pop(-1)
    ...
    def pause(self, event=None, reset=False):
        ...
        event = event or self.buttons["Pause"]
